...
Remember that Basic++ uses [semantic versioning], so if you are upgrading
from one major release version to another, please carefully study the
information provided on [versioning] of Basic++.
...
=== "with `pip`"
    ...
    ```
    pip install --upgrade --force-reinstall basic-pp
    ```
...
Basic++ follows [semantic versioning] and currently uses **0.0.x versioning**
